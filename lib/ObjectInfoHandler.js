...
class NexacroMetaObjectInfoHandler extends NexacroMetaInfoHandler {
    ...
    #parsePropertyInfo (propertyNode) {
        ...
        if (propertyNode && propertyNode.$) {
            ...
            if (fix_property_type.hasOwnProperty(prop_attrs.name)) {
        ...
        if (methodNode && methodNode.$) {
            ...
            if (method_syntax.Return && method_syntax.Return.length > 0) {
                ...
                if (return_attrs) {
                    ...
                    if (fix_method_returntype.hasOwnProperty(method_attrs.name)) {
            ...
            if (method_syntax.Arguments && method_syntax.Arguments.length > 0) {
                ...
                // parsing syntax text
                let syntax_text = method_syntax.$.text, objType = 'object', einfoType = 'nexacro.EventInfo';
                    ...
                    syntax_texts.forEach((syntax_) => {
                        ...
                        match_args.forEach((argstr) => {
                            ...
                            if (found_arg) {
                                ...
                                if (fix_method_argtype.hasOwnProperty(method_attrs.name)) {
                                    const fix_info = fix_method_argtype[method_attrs.name];
                                    
                                    if ( fix_info.hasOwnProperty(arg_attrs.name)) {
    ...
    #toInterfaceCode (interfaceName, extendTypeName, ctorCode, bodyCodes, indent) {
...
`;
    ...
    #generateInterfaceCodes (option, indent) {
        ...
        const interfaceCode = this.#toInterfaceCode(interfaceName, extendTypeName, ctorCode, bodyCodes, indent);
